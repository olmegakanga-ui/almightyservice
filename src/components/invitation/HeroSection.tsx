...
function formatEventDate(isoString: string) {
  ...
  // Lire l'heure directement depuis la chaîne ISO — pas de conversion UTC
  ...
  return (
    ...
    >
      ...
      <div className="relative z-20 text-center max-w-4xl mx-auto w-full">
        ...
        {/* Nom de l'invité */}
        <div style={{ opacity: mounted ? 1 : 0, transform: mounted ? 'translateY(0)' : 'translateY(30px)', transition: 'all 1s cubic-bezier(0.16,1,0.3,1) 1.1s', marginBottom: '48px' }}>
        ...
        <div style={{ opacity: mounted ? 1 : 0, transform: mounted ? 'translateY(0)' : 'translateY(20px)', transition: 'all 0.9s cubic-bezier(0.16,1,0.3,1) 1.4s', marginBottom: '60px' }}>
          ...
          >
            <div className="text-center">
              ...
              <p style={{ color: 'var(--gold-light)', fontSize: '0.9rem', fontFamily: 'var(--font-display)', fontWeight: 500 }}>{day}</p>
            </div>
            <div style={{ width: '1px', height: '32px', background: 'var(--gold-border)', flexShrink: 0 }} />
            <div className="text-center">
              <p className="label-overline mb-1">Date</p>
              <p style={{ color: 'var(--gold-light)', fontSize: '0.9rem', fontFamily: 'var(--font-display)' }}>{date}</p>
            </div>
            <div style={{ width: '1px', height: '32px', background: 'var(--gold-border)', flexShrink: 0 }} />
            <div className="text-center">
              <p className="label-overline mb-1">Heure</p>
              <p style={{ color: 'var(--gold-light)', fontSize: '0.9rem', fontFamily: 'var(--font-display)' }}>{time}</p>
            </div>
            {guest.tableName && (
              <>
                <div style={{ width: '1px', height: '32px', background: 'var(--gold-border)', flexShrink: 0 }} />
                <div className="text-center">
                  ...
                  <p style={{ color: 'var(--gold-light)', fontSize: '0.9rem', fontFamily: 'var(--font-display)' }}>{guest.tableName}</p>
                </div>
              </>
            )}
          </div>
