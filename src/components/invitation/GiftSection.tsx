...
export default function GiftSection({ guestId, eventId, initialChoice, giftOptions }: Props) {
  ...
  // Filtrer les options selon la config admin
  const options = ALL_OPTIONS.filter(o => giftOptions.includes(o.id))

  // Si une seule option — pré-sélectionner
  useEffect(() => {
  ...
  // Si aucune option activée → ne rien afficher
    ...
    <section ref={sectionRef} style={{ padding: '100px 24px', position: 'relative' }}>
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: '56px' }}>
          ...
          <p className="label-overline" style={{ marginBottom: '12px' }}>Votre présent</p>
        ...
        <div style={{
          ...
          gap:                 '16px',
          maxWidth:            options.length === 1 ? '320px' : '100%',
          margin:              '0 auto',
        }}>
          {options.map((opt, i) => {
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border:         isSel ? '1px solid rgba(201,169,110,0.5)' : '1px solid rgba(255,255,255,0.07)',
                  cursor:         'pointer',
              ...
              >
                <div style={{
                  ...
                  border:         isSel ? '1px solid rgba(201,169,110,0.4)' : '1px solid rgba(255,255,255,0.08)',
                  display:        'flex',
                  alignItems:     'center',
                  justifyContent: 'center',
                ...
                <div style={{ textAlign: 'center' }}>
                  <p className="font-display" style={{ fontSize: '1.3rem', color: isSel ? 'var(--gold-light)' : 'rgba(255,255,255,0.7)', marginBottom: '4px' }}>
                    {opt.label}
                  </p>
                  <p style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.3)' }}>{opt.sub}</p>
                </div>
                <div style={{
                  width:          '20px',
                  height:         '20px',
                  ...
                  {isSel && (
                    <svg width="10" height="10" viewBox="0 0 10 10">
                      <path d="M2 5l2.5 2.5L8 3" stroke="#0D0B09" strokeWidth="1.5" strokeLinecap="round" fill="none"/>
                    </svg>
                  )}
                </div>
              </button>
